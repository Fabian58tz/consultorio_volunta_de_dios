document.addEventListener("DOMContentLoaded", function() {
    ...
    // (Esta sección no cambia ya que la funcionalidad del sidebar es independiente de la tabla de historial)
    ...
    // --- Funcionalidad de la Tabla de Historial ---

    // Función para añadir una nueva fila
    addRowButton.addEventListener('click', function() {
        const newRow = citasTableBody.insertRow();
        newRow.setAttribute('data-id', nextId);
        // HTML de la nueva fila adaptado a las columnas de "Tabla de historial"
        newRow.innerHTML = `
            <td class="editable-cell"><input type="text" value=""></td>
            <td class="editable-cell"><input type="text" value=""></td>
            <td>${new Date().toLocaleDateString('es-ES')}</td> <td><span class="status-activo">Activo</span></td>
    ...
    function enableRowEditing(row) {
        // Ahora solo apuntamos a los primeros DOS elementos td con la clase 'editable-cell'
        // (Nombre del paciente, Otros datos)
        Array.from(row.querySelectorAll('td.editable-cell')).slice(0, 2).forEach(cell => {
